...
async def login(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    is_local = os.getenv("ENVIRONMENT", "dev").lower() in ("dev", "local", "development")
    if is_local:
        callback_url = f"{backend_url}/api/v1/auth/callback?code=mock_code&state={state}"
        logger.info("[login] Local dev mode: redirecting straight to callback: %s", callback_url)
...
async def callback(
    ...
    is_local = os.getenv("ENVIRONMENT", "dev").lower() in ("dev", "local", "development")
    if is_local:
        # Local mock user token issuance directly
        user = User(id="mock_user_123", email="mock_user@example.com", name="Mock User", role="user")
        ...
        logger.info("[callback] Mock login successful, redirecting to %s", redirect_url)
        return RedirectResponse(
            url=redirect_url,
            status_code=status.HTTP_302_FOUND,
        )
...
async def logout():
    ...
    is_local = os.getenv("ENVIRONMENT", "dev").lower() in ("dev", "local", "development")
    if is_local:
        logout_url = f"{settings.frontend_url}/logout-callback"
    else:
