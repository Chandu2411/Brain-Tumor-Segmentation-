...
class Segmentation_resultsData(BaseModel):
    ...
    filename: str
    # Honest inference statistics
    tumor_detected: Optional[bool] = None
    tumor_pixel_count: Optional[int] = None
    tumor_coverage_percentage: Optional[float] = None
    mean_tumor_probability: Optional[float] = None
    max_tumor_probability: Optional[float] = None
...
class Segmentation_resultsResponse(BaseModel):
    """
    API response for a segmentation result.

    Contains honest inference statistics only.
    Dice / IoU are absent because they require ground-truth masks.
    Use POST /api/evaluate to compute Dice / IoU.
    """
    id: int
    user_id: str
    original_image_key: str
